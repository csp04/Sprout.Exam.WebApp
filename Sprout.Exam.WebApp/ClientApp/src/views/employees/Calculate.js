...
export class EmployeeCalculate extends Component {
  ...
  handleSubmit(e) {
    ...
    if (!this.isValidNumber(this.state.absentDays)) {
      alert('Invalid input: ' + this.state.absentDays);
      return;
    }

    if (!this.isValidNumber(this.state.workedDays)) {
      alert('Invalid input: ' + this.state.workedDays);
      return;
    }
      ...
      <div>
        <form>
          ...
          <div className="form-row">
            <div className="form-group col-md-12">
              <label>
                Net Income: <b>{formatCurrency(this.state.netIncome)}</b>
              </label>
            </div>
          </div>
